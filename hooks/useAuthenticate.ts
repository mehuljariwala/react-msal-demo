...
export const useAuthenticate = ({
  ...
  const [authenticationInProgress, setAuthenticationInProgress] =
    useState(false);
  const [authenticationClientInitialized, setAuthenticationClientInitialized] =
    useState(false);
  ...
  useEffect(() => {
    const initiateClientAndMaybeAuthenticateSilently = async () => {
      ...
      if (authenticationClientExists()) {
        setAuthenticationClientInitialized(true);
      }
  ...
  return {
    ...
    authenticate: () =>
      void withAuthenticationPromiseHandler(
        authenticateInteractively(scopes),
        "MANUAL"
      ),
